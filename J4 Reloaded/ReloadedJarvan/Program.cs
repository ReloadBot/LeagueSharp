...
namespace StonedJarvan
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Game.PrintChat("<font color='#000080'>J4Reloaded By</font> <font color='#B22222'>ReloadBot</font>");
        }
        ...
        private static void OnGameUpdate(EventArgs args)
        {
            ...
            if (Player.Distance(target,false) <= 600 && IgniteDamage(target) >= target.Health && Config.Item("Ignite").GetValue<bool>())
            {
        ...
        private static void Combo()
        {
            ...
            if (E.IsReady() && Q.IsReady())
            {
                if (Player.Distance(target,false) <= E.Range && (Config.Item("UseEQCombo").GetValue<bool>()))
                {
                    E.Cast(target);
                    Q.Cast(target);
                }
            }
            else if (Player.Distance(target,false) <= E.Range && E.IsReady() && (Config.Item("UseECombo").GetValue<bool>()))
            {
                E.Cast(target);
            }
            else if (Player.Distance(target,false) <= Q.Range && Q.IsReady() && (Config.Item("UseQCombo").GetValue<bool>()))
            {
                Q.Cast(target);
            }
            if (Player.Distance(target,false) <= R.Range && (Config.Item("UseRCombo").GetValue<bool>()))
            {
                R.Cast(target);
            }
            if (Player.Distance(target,false) <= W.Range && (Config.Item("UseWCombo").GetValue<bool>()))
            {
                W.Cast();
            }
